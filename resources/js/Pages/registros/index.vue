...
const eliminar = (id) => {
  if (confirm('¿Eliminar este registro?')) {
    router.delete(route('registros.destroy', id))
  }
}
</script>
...
<template>
  <AuthenticatedLayout>
    <div class="container mx-auto p-6">
      ...
      <h1 class="text-2xl font-bold mb-6 tituloPag">Lista de Registros</h1>

      <!-- Mensaje de éxito -->
      <div v-if="flash" class="bg-green-100 text-green-700 p-3 rounded mb-4">
        {{ flash }}
      </div>

      <!-- Botón para crear registro -->
      <div class="mb-4">
        <a :href="route('registros.create')" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded shadow transition">
          + Agregar
        </a>
      </div>
          ...
          <tbody>
            <tr v-for="registro in registros" :key="registro.id" class="hover:bg-gray-50">
              ...
              <td class="py-2 px-4 border-b flex justify-center gap-2">
                ...
                <!-- Eliminar -->
                <button @click="eliminar(registro.id)"
